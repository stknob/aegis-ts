...
async function bench_aegis128l() {
    ...
    for (const [size, samples] of PARAMS) {
        const [ct, tag] = aegis128l(key, nonce, ad).encryptDetached(new Uint8Array(size));
        await bench(`aegis128l @ ${size}`, samples, () => aegis128l(key, nonce, ad).decryptDetached(ct, tag));
    }
};
...
async function bench_aegis256() {
    ...
    console.log("--------------------------------");
    for (const [size, samples] of PARAMS) {
        const [ct, tag] = aegis256(key, nonce, ad).encryptDetached(new Uint8Array(size));
        await bench(`aegis256 @ ${size}`, samples, () => aegis256(key, nonce, ad).decryptDetached(ct, tag));
    }
}
...
async function bench_aegis128l_misaligned() {
    ...
    for (const [size, samples] of PARAMS) {
        ...
        await bench(`aegis128l misaligned nonce + ct @ ${size}`, samples, () => aegis128l(key, nonce, ad).decryptDetached(ct, tag));
    }
}
...
async function bench_aegis256_misaligned() {
    ...
    for (const [size, samples] of PARAMS) {
        ...
        const ct_unaligned = new Uint8Array(ct.length + 1).subarray(1); ct_unaligned.set(ct);
        const nonce_unaligned = new Uint8Array(nonce.length + 1).subarray(1); nonce_unaligned.set(nonce);
        await bench(`aegis256 misaligned nonce + ct @ ${size}`, samples, () => aegis256(key, nonce, ad).decryptDetached(ct, tag));
    }
}
