import { clean, u32_aligned_at, u8 } from "./_utils.mjs";
...
export class MD5 {
    ...
    #transform(msg: Uint8Array, offset: number, nblocks: number) {
        const msg32 = u32_aligned_at(msg, offset, nblocks << 6 /* 64 bytes per block */);
        const x = new Uint32Array(16);
        for (let i = 0, off = 0; i < nblocks; i++, off += 16) {
            const [a, b, c, d] = this.#state;
            x.set(msg32.subarray(off, off + 16));
    ...
    update(msg: Uint8Array): this {
        ...
        if (msg.length >= partlen) {
            ...
            this.#transform(this.#buf, 0, 1);
            msg_off += partlen;
            index = 0;

            const nblocks = (msg.length - msg_off) >>> 6;
            if (nblocks > 0) {
                this.#transform(msg, msg_off, nblocks);
                msg_off += nblocks << 6;
            }
        }
